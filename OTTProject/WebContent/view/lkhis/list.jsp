		...
		<article id="main">
			...
			<section class="wrapper style5">
				<div class="inner" style="text-align: center;">
					<div class="wisimg">
							<%
								int lkhisSize = lkhisList.size();
							if (lkhisSize > 0) {
								for (int i = 0; i < lkhisSize; i++) {
							%>
							<div>
								...
								<input class="form-check-input chkbox" type="checkbox" name="check"
										...
										value="<%=lkhisList.get(i).getLkhisNum()%>">
									<a href="/OTTProject/main/detail.do?vdodtlNum=<%=lkhisList.get(i).getVdodtlNum()%>"><img class="ima1" src="/OTTProject/hsImages/<%= lkhisList.get(i).getStreFileNm() %>"></a>
								</div>
								
								<div class="title">
									<a class = "wistitle" href="/OTTProject/main/detail.do?vdodtlNum=<%=lkhisList.get(i).getVdodtlNum()%>"><%=lkhisList.get(i).getVdodtlNm()%></a><br>
									좋아요누른일<%=lkhisList.get(i).getLkhisDown()%>
								</div> 
	...
	<script>
		...
		function f_del(){
		   ...
		   $("input[name=check]:checked").val((p_index,p_value)=> {
		      queryString += "lkhisNum="+p_value+"&";
// 			   alert(queryString);
		   });
	
		   $.ajax({
		      type:"get",
		      url:"<%=request.getContextPath()%>/lkhis/remove.do",
		      data: queryString,
		      dataType:"text",
		      success:function(result){
		         if(result == "성공"){
		    	  console.log(queryString);
		            location.href= "<%=request.getContextPath()%>/lkhis/list.do";
		         }
		      },
